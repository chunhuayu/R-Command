...
# In this exercise you'll use the mtcars data frame to explore typical variations of simple histograms. 
# But first, some background:

# The x axis/aesthetic: The documentation for geom_histogram() states the argument stat = "bin" as a default. 
# Recall that histograms cut up a continuous variable into discrete bins - that's what the stat "bin" is doing. 
...
# Actually, there is a variable mapped to the y aesthetic, it's called ..count... 
# When geom_histogram() executed the binning statistic (see above), it not only cut up the data into discrete bins,
# but it also counted how many values are in each bin. So there is an internal data frame where this information is stored. 
# The .. calls the variable count from this internal data frame. This is what appears on the y aesthetic. But it gets better!
# The density has also been calculated. This is the proportional frequency of this bin in relation to the whole data set.
