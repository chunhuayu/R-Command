...
### The resulting object (which I've named z) is an lm object containing all the results. 
### You use additional commands to pull out these results. 
### Here are some of the most useful commands to extract results from the lm object:
...
### The simplest linear model fits a constant, the mean, to a single variable. 
### This is useful if you want to obtain an estimate of the mean with a standard error and confidence interval, 
### or wanted to test the null hypothesis that the mean is zero.
...
### To fit a linear regression model without an intercept term, 
### i.e., a regression through the origin, add "-1" to the model statement:
...
### Analysis of covariance models include both numeric and categorical variables. 
### The linear model in this case is a separate linear regression for each group of the categorical variable. 
### Interaction terms between these two types of variables, if included in the model, fit different linear regression slopes; 
### otherwise the same slope is forced upon every group.
z <- lm(y ~ x + A + x:A, data = mydata)

### Simple linear regression Fit the model to data:
z <- lm(y ~ x, data = mydata)
